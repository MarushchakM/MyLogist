import { fetchTrailers, selectTrailerLoading, selectTrailersNumbersAndIds } from "@/lib/features/trailers/trailersSlice";
import { fetchTrucks, selectTruckNumbersAndIds, selectTrucksLoading } from "@/lib/features/trucks/trucksSlice";
...
export const SubMenu: React.FC<Props> = ({sectionId}) => {
  ...
  const data = useAppSelector(state => {
    switch (sectionId) {
      case 'trucks': return selectTruckNumbersAndIds(state);
      case 'trailers': return selectTrailersNumbersAndIds(state);
    }
  })
  ...
  return (
    <ul className={style.subMenu}>
      {isLoading ? (
        <li className={style.spinner}><Spinner /></li>
      ) : (
